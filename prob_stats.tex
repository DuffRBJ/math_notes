...
%%% Document Starts now
\begin{document}
\title{Probability, Statistics, Machine Learning}
\begin{thm}
If $ X $ and $ Y $ are random variables, then the joint pdf of $ X+Y $ is given by convolution:
$$ f_{X+Y}(t) = \int_{-\infty}^{\infty} f_X(z)f(t-z)  dz$$ 
\end{thm}

\begin{defn}
    The uniform distribution on $ [a,b] $ is
    $$ U(a,b) = \frac{1}{b-a}\boldsymbol{1}_{[a_b]}$$ 
\end{defn}
